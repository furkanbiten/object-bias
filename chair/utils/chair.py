...
class CHAIR(object):
    ...
    def get_annotations(self):
        ...
        '''
        anns_path = '../data/anns_coco.pkl'
        if os.path.exists(anns_path):
            self.imid_to_objects = pickle.load(open(anns_path, 'rb'))
        else:
            self.get_annotations_from_segments()
            self.get_annotations_from_captions()
            pickle.dump(self.imid_to_objects, open(anns_path, 'wb'))
...
def load_generated_captions(cap_file):
   ...
   try:
       ...
       # imids = set([cap['image_id'] for cap in caps.values()])
...
def save_hallucinated_words(cap_file, cap_dict): 
    ...
    OUT_PATH = os.path.join(*cap_file.split('/')[:-1], 'output')
    if not os.path.exists(OUT_PATH):
        os.makedirs(OUT_PATH)
    with open(os.path.join(OUT_PATH, 'hallucinated_words_%s' %tag), 'w') as f:
