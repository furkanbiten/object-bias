...
def language_eval(dataset, preds, preds_n, eval_kwargs, split):
    if eval_kwargs['id'] == '':
        model_id = eval_kwargs['checkpoint_path'].split('/')[1]
    else:
...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        if eval_kwargs.get('use_ft', None) and not eval_kwargs.get('label_aligned', None):
            tmp = [data['fc_feats'], data['att_feats'], data['ft_feats'], data['labels'], data['masks'],
                   data['att_masks'], data['ft_masks']]
            tmp = [_ if _ is None else _.cuda() for _ in tmp]
            fc_feats, att_feats, ft_feats, labels, masks, att_masks, ft_masks = tmp
            att_feats = (att_feats, ft_feats)
            att_masks = (att_masks, ft_masks)
        else:
    ...
    torch.save((predictions, n_predictions), os.path.join('eval_results/', '.saved_pred_'+ model_id + '_' + split + '.pth'))
    if lang_eval == 1:
