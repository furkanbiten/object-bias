...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.use_ft = self.use_ft and not self.aligned
        ...
        if self.use_ft:
            self.ft_size = getattr(opt, 'ft_size', 0)
            self.att_embed_ft = nn.Sequential(*(
                    ((nn.BatchNorm1d(self.ft_size),) if self.use_bn else ()) +
                    (nn.Linear(self.ft_size, self.rnn_size),
                     nn.ReLU(),
                     nn.Dropout(self.drop_prob_lm)) +
                    ((nn.BatchNorm1d(self.rnn_size),) if self.use_bn == 2 else ())))
            self.ctx2att_ft = nn.Linear(self.rnn_size, self.att_hid_size)
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        if self.use_ft:
            att_feats_ft, att_masks_ft = att_feats[1], att_masks[1]
            att_feats, att_masks = self.clip_att(att_feats[0], att_masks[0])
            att_feats_ft, att_masks_ft = self.clip_att(att_feats_ft, att_masks_ft)

            # embed fc and att feats
            fc_feats = self.fc_embed(fc_feats)
            att_feats = pack_wrapper(self.att_embed, att_feats, att_masks)
            att_feats_ft = pack_wrapper(self.att_embed_ft, att_feats_ft, att_masks_ft)

            # Project the attention feats first to reduce memory and computation comsumptions.
            p_att_feats = self.ctx2att(att_feats)
            p_att_feats_ft = self.ctx2att_ft(att_feats_ft)

            # Concatanate the features
            p_att_feats = torch.cat([p_att_feats, p_att_feats_ft], dim=1)
            att_feats = torch.cat([att_feats, att_feats_ft], dim=1)
            att_masks = torch.cat([att_masks, att_masks_ft], dim=1)
        else:
...
class UpDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        # TODO: change fc_feats so that it takes into account fastText features
        prev_h = state[0][-1]
